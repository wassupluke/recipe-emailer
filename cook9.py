...
def scrape_by_h2(soup, y, rb):
    ...
    for a in recipes:
        ...
        if "plan" in link.lower():
            ...
            link = "https://leanandgreenrecipes.net{}".format(link) 
            rb.append(link)
        else:
            rb.append(link)
    return rb
...
if __name__ == "__main__":
    ...
    pretty = "\n\n".join(all_recipes)
    total_sites_line = f"<p style=color:gray>Wowza! We found {len(recipebook) + len(scraperbook)} recipes! These {len(meals)} were selected at random for your convenience and your family's delight. version 9</p>"
    pretty = f"{pretty}\n\n{total_sites_line}"
